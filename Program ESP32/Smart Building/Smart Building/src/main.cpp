...
#include <ZMPT101B.h>
#include <ACS712.h>
#include <WebServer.h>          // Server Web untuk ElegantOTA
...
WebServer server(80); // ElegantOTA menggunakan server ini
...
void setup() {
  ...
  wifiManager.autoConnect("Griya_Tech");

  if (!WiFi.isConnected()) {
    Serial.println("Failed to connect to WiFi. OTA will not work.");
  } else {
  ...
  server.begin();
  Serial.println("ElegantOTA ready! Access at: http://" + WiFi.localIP().toString() + "/update");
}
...
void loop() {
  server.handleClient(); // Menangani permintaan dari ElegantOTA
  ...
  // Interval untuk loop utama
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis;

    // Tambahan kode utama Anda
      float humidity = 0;
      float temperature = 0;
      dht11.update();
      // Membaca kelembaban
